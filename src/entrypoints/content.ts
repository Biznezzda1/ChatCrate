...
export default defineContentScript({
    ...
    const waitForElement = (selector: string, timeout = 3000): Promise<Element | null> => {
      return new Promise((resolve) => {
        ...
        const observer = new MutationObserver(() => {
          const element = document.querySelector(selector);
          if (element) {
            console.log(`[ChatCrate Content] Element found after mutation: ${selector}`);
            observer.disconnect();
            resolve(element);
          }
        });
  ...
  if (message.action === 'getPageContext') {
    (async () => {
          try {
            ...
            // 检测页面类型
            let pageType: 'search' | 'deepresearch' | 'labs' | 'unknown' = 'unknown';
            
            // 1. 检查 URL 是否为 /search/<id>
            console.log('[ChatCrate Content] Checking if pathname starts with /search/');
            if (pathname.startsWith('/search/')) {
              ...
              // 检测所有可能的元素
              const researchTab = document.querySelector('[data-testid="answer-mode-tabs-tab-research"]');
              const labsTab = document.querySelector('[data-testid="answer-mode-tabs-tab-studio"]');
              
              // 调试：列出所有带 data-testid 的按钮
              const allButtons = document.querySelectorAll('button[data-testid*="tab"]');
              console.log('[ChatCrate Content] All tab buttons found:', allButtons.length);
              allButtons.forEach((btn, idx) => {
                console.log(`[ChatCrate Content] Button ${idx}:`, btn.getAttribute('data-testid'));
              });
            ...
            const hasContent = document.querySelector('[data-testid="answer-content"]') || 
                              document.querySelector('h1') ||
                              document.body.textContent!.length > 100;
            
            const loadingState: 'loading' | 'complete' = hasContent ? 'complete' : 'loading';
    ...
    (async () => {
      try {
        ...
        try {
          ...
          console.error('[ChatCrate Content] Clipboard API failed:', clipboardError);
  ...
  if (message.action === 'ping') {
    ...
    console.log('[ChatCrate Content] ===== Message listener set up successfully =====');
  }
});
